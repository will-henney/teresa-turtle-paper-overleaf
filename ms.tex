...
\begin{table*}
  ...
  \begin{tabular}{
    ...
    {(1)} & {(2)} & {(3)} & {(4)} & {(5)} & {(6)} & {(7)} & {(8)} & {(9)}\\
    \addlinespace
    Inner shell & 44\% & 4000--5000 & 0.078 & 0.0036--0.0070 & -4 & -1.5 & 3.1e-5 & 40--55\\
...
Also, this method can only estimate mass loss before \SI{1500}{yr} ago, which is the kinematic timescale of the inner shell.
...
Sub-arc second imaging observations of young planetary nebulae \citep{Sahai:1998a, Sahai:2011a}
and preplanetary nebulae \citep{Sahai:2007a}
show that such asymmetries can appear very early in the evolution of the nebula,
before the central star is hot enough to emit ionizing photons.
...
Outside of this nested shell structure lie two further highly anisotropic outflow systems with very different characteristics:
the lobes and the knot complexes.  The lobes are highly 
...
The outermost component of NGC~6210 is the halo
...
extending from roughly \num{0.1} to \SI{0.5}{pc} from the central star
The components of NGC~6210 can be divided 

The halo is not a recombining one like \citet{Corradi:2000a}


ADD is of order 3 for N, O, and Ne \citep{Bohigas:2015c}
...
\item Optically thin nebula: ratio of \ion{He}{1} to \ion{H}{1} Zanstra temperature \(\ge 1.3\) \citep{Phillips:2003c}.
\item Within a distance of \SI{4}{kpc} of the Earth
  (distances are taken from \citealp{Frew:2016a}). 
\item Similar nebular physical size to NGC~6210: \(\SI{0.08}{pc} < R < \SI{0.2}{pc}\).
\item Central star with H-rich spectral classification, principally O(H). \citep{Weidmann:2020a}
\end{enumerate}
This yields 5 sources in addition to the Turtle:
NGC~1535, IC~368, Jonckheere~320,  NGC~5307, and NGC~6326.
