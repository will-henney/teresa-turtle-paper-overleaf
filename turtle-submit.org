...
*** TODO Move Affinity Designer files to this repo
...
#+begin_src sh :results verbatim
  files=$(grep 'Graphic file' ms.log | cut -d ' ' -f2 | grep "^figs/" | grep ".pdf$" | sed -e 's/-lores//'| sed -e 's/pdf/afdesign/' | sed -e 's/figs/..\/Teresa-Turtle\/figs/')
  for f in $files; do
      cp -v $f figs
  done
#+end_src
